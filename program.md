...
**trade_v3 效率质量版**，目标是从已收敛的 IC/权重微调阶段，转向超额质量、
收益/回撤效率、低换手和新因子结构。
...
3. **低换手与稳定性改造**
   ...
   - 优先提高 Top 篮子 Jaccard、降低 `annual_turnover`，同时守住 IC。
4. **组合方法升级，谨慎使用 ML**
...
6. 若出现 `score_anomaly`、连续 3 次 CRASH、API 连续失败、或生成内容不包含有效 `run()` / `ITER_NOTE`，
   服务必须进入暂停/等待人类状态，而不是继续消耗迭代预算。
7. 连续服务不得把 `journal/`、`factor_library/`、`service_state/` 中的历史分数作为特征或训练数据；
   这些只用于审计、展示和人工复盘。
...
- 财务因子、行业字段：仍不开放
- 当前 best：`run_0147_score_p5.4282.py`，H=20，LABEL_KIND=`rank`
- 阶段目标：不是重启研究，而是在现有 best 上继续寻找低相关、低换手、提高收益/回撤效率的增量。
...
- 增加一个与现有因子相关性低的新因子；
- 对新因子做正交化后再加入；
- 轻量降低换手，观察 `trade_v3` 下收益/回撤效率是否提升。
...
在线服务遇到 `score_anomaly` 时，必须把状态切到“等待人类复核”，并在交付日志中突出展示：

- 被拒绝实验的 score 与当前 best score
- Sharpe / 年化 / 回撤 / 换手率的改善幅度
- 是否建议人类调整 `prepare.primary_score`
- 是否值得把该实验作为“产品质量候选”另行保留
