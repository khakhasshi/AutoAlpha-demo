...
## 3. 研究方向（Phase 3：稳健性与反过拟合）

当前 best 已在 `HORIZON=20 / LABEL_KIND=rank / IC 衰减权重 / 低换手自适应平滑`
附近收敛。#0219 的 `slow_vol_regime_60` 是本阶段最大结构性突破，#0220 的去冗余
消除了同质化惩罚，#0250/#0262/#0263 的自适应平滑把交易质量继续推高。

这说明当前不再是“继续找一个更强平滑参数”的阶段。后续不要把主要预算花在：
半衰期、窗口、标签口味、平滑 span、rolling median window、或继续叠加平滑层的细碎微调上。
这些可能继续在 val 上小幅加分，但真实价值递减，且会提高验证段适配风险。
...
1. **稳健性/消融优先**
   - 优先验证 #0250/#0262/#0263 的平滑层是否存在重复：能删一层而大部分指标不掉，就是更有价值的生产候选。
   - 允许在 `alpha.py` 内做简化式消融，例如删除低价值平滑层、降低代码复杂度、降低参数自由度。
   - 消融不是为了提高 val score，而是为了找更稳的结构；若 score 小幅下降但逻辑更稳，需要在 `ITER_NOTE` 写清楚。
2. **低相关新结构，而不是窗口换皮**
   - 新因子必须解释它相对 `slow_vol_regime_60`、`momentum_20`、`price_range_position_10` 的新信息。
   - 优先价量背离、流动性冲击后的恢复、慢变量状态切换、正交残差。
   - 不要再堆叠 20/60/120/252 的同类窗口版本，除非做删换而不是新增。
3. **正交化/去冗余组合**
   - 新因子先对现有组合或高度相关因子做截面残差化，再进入组合。
   - 优先保留与现有因子相关性低、但能改善超额 Sharpe 或效率的因子。
   - 若新增因子只带来相关性惩罚，应优先删除或替换，而不是继续堆叠。
4. **低换手与稳定性改造需克制**
   - 当前换手和 Top basket Jaccard 已经较低/较高，继续降换手的边际价值下降。
   - 只允许一个候选 proposal 涉及平滑，并且必须说明为什么不是简单 span/window 微调。
   - 守住 IC、Sharpe、收益/回撤效率比继续降低换手更重要。
5. **组合方法升级，谨慎使用 ML**
